...
func (r *AuthRepo) Register(req *pb.RegisterReq) (*pb.RegisterRes, error) {
	...
	query := `INSERT INTO users(username, face_id, gym_id, phone_number, email, password, full_name, date_of_birth) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`
	err = tr.QueryRow(query, req.Username, req.FaceId, req.GymId, req.PhoneNumber, req.Email, req.Password, req.FullName, req.DateOfBirth).Scan(&id)
	if err != nil {
		tr.Rollback()
		return nil, err
	}
